def compute_opm(opm_list: list):
    try:
        ...
        for i in range(1, len(opm_list)):
            if ((opm_list[i - 1] - opm_list[i]) / max(abs(opm_list[i]), 0.01) <
                    -0.2):
                drop = 1
                break
        ...
        if (opm_sum < 0 or opm_list[0] < 0):
            ...
            ((opm_list[0] - opm_list[1]) / max(abs(opm_list[1]), 0.01)) < -0.2
                or (drop == 0 and opm_sum < 20)):
            return "B"
        elif (drop == 1):
            return "BB"
        elif (opm_sum > 60 or
              (opm_sum >= 40 and opm_list[0] > opm_list[1] and opm_sum <= 60)):
            return "AA"
        elif (opm_sum > 40 or
              (opm_sum >= 20 and opm_list[0] > opm_list[1] and opm_sum <= 40)):
            return "A"
        else:
            return "BB"
        return "error"
...
def compute_argr(argr_list: list):
    try:
        ...
        if (argr_sum < 0 or argr_list[0] < 0):
            return "C"
        if (argr_sum >= 0 and argr_list[0] < argr_list[1]
              and argr_list[1] < argr_list[2]):
            return "B"
        if (negative > 0):
            return "BB"
        if (negative == 0 and argr_sum >= 150
                and argr_list[0] >= argr_list[1]):
            return "AA"
        elif (negative == 0 and (argr_sum >= 60
               and argr_list[0] >= argr_list[1] and argr_sum < 150)
              or (argr_sum >= 150 and argr_list[1] - argr_list[0] <
                  max(abs(argr_list[1]), 0.01) * 0.5)):
            return "A"
        else:
            return "BB"
        return "error"
...
def compute_niatgr(niatgr_list: list):
    try:
        list1 = []
        for i in range(0, 4):
            list1.append((niatgr_list[i] - niatgr_list[i + 4]) /
                         max(abs(niatgr_list[i + 4]), 0.01))
        negative = 0
        fifty = 0
        positive = 0
        for i in range(len(list1)):
            if (list1[i] < 0):
                negative += 1
            if (list1[i] > 0.5 and i != len(list1) - 1):
                fifty += 1
        ...
        if (list1[0] < 0 and list1[1] < 0):
            return "C"
        elif (list1[0] < 0 or negative >= 2
              or (list1[0] < list1[1] and list1[1] < list1[2] and fifty == 0)):
            return "B"
        elif ((list1[0] >= 0 and list1[1] >= 0
               and list1[1] - list1[0] >= max(abs(list1[1]), 0.01) * 0.5)
              or (list1[1] < 0 and list1[0] >= 0)):
            return "BB"
